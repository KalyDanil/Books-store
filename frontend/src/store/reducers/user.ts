...
('users/uploadAvatar',
  async (file: {}, { dispatch, getState }) => {
    ...
    try {
      const res: {} = await api.post('http://localhost:4000/user/avatarUpload', file, config);
      return res
    } catch(err: any) {
      console.log(err)
      return err
...
export const userSlice = createSlice({
  ...
  initialState,
  reducers: {
    authorizationAction: (state, action: PayloadAction<{id: number, fullName: string, email: string, avatar: string | null | undefined, tokenIsValid?: boolean}> ) => {
    ...
    nameChangerAction: (state, action: PayloadAction<string> ) => {
...
export const {authorizationAction, avatarUploadAction, refusalAction, nameChangerAction} = userSlice.actions
